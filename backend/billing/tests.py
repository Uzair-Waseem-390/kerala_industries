...
from purchases.models import Family, Product, PurchaseReturn, Shelf
...
class FIFOReversalRepeatReturnTests(BillingTestBase):
    ...
    per-batch remaining_quantity even though the aggregate nets out. This
    is the exact drift class the 2026-09 WIP/FG opening-stock bug also
    produced, just one layer deeper (per-batch, not per-account).
    """
    ...
    def make_two_batch_product(self):
        ...
        """
        product = Product.objects.create(
            name="Two Batch Product", code="TBP",
