...
from inventory.models import ProductRegistryEntry, ShelfStockMovement, WipShelfStockMovement
from inventory.services import apply_shelf_allocations as apply_rm_shelf_allocations
from inventory.services import apply_wip_shelf_allocations, create_registry_entry, sync_wip_inventory
...
def add_breakdown_item(*, recipe_id: int, yard_value: Decimal, quantity: Decimal, shelf_allocations: list[dict], user) -> RecipeBreakdownItem:
    ...
    # this batch's cost isn't known yet (unit_cost_snapshot/
    # full_unit_cost_snapshot are only computed in finish_recipe, once
    # labor/machines/the DL+FOH pool are known). Adding stock before its
    # cost exists would force avg_unit_cost to either skip this batch's
    # cost forever or be patched in after the fact against a moving
    # target — moved the whole put-away (sync_wip_inventory +
    # apply_wip_shelf_allocations) to finish_recipe instead, right after
    # cost is stamped, so quantity and cost land together in one place,
    # same shape as a real purchase. RecipeBreakdownItemShelfAllocation
    # rows are still recorded now (the chosen shelf split), just not
    # applied to real ShelfStock until finish.
...
def finish_recipe(*, recipe_id: int, user) -> Recipe:
    ...
    breakdown_items = list(recipe.breakdown_items.prefetch_related("shelf_allocations").all())
    ...
    # Put away each item's stock now that its cost is known — quantity and
    # cost land together, same shape as a real purchase (see
    # add_breakdown_item's docstring above for why this moved here).
    for item in breakdown_items:
        sync_wip_inventory(
