...
import { Plus, Package, SlidersHorizontal, X } from 'lucide-react';
import { usePackingRecipes } from '../../hooks/useProduction';
...
// Recipe list — Packing is the third and final WIP stage (Cut Pieces + RM
// Packing Material -> Finished Goods). Mirrors CuttingRecipesPage exactly;
// the create form only collects name + description since recipe_type is
// implied by the /packing-recipes/ endpoint itself.
const PackingRecipesPage = () => {
    ...
    const handleCreate = async (e) => {
        ...
        try {
            ...
            resetForm();
            if (created?.id) {
                navigate(`/production/packing-recipes/${created.id}`);
            }
        } catch (err) {
        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-2xl sm:text-3xl font-bold text-neutral-900">Packing Recipes</h1>
                        <p className="text-neutral-500 mt-0.5 text-sm sm:text-base">
                            Packing production batches — issue a Cut Piece and Packing Material, finish to produce Finished Goods
                        </p>
                    </div>
                </div>
                <Button
            ...
            <Card className="p-0 overflow-hidden" hover={false}>
                {recipes.length === 0 ? (
                    <EmptyState
                        ...
                        title="No packing recipes found"
                        description={searchTerm || Object.keys(filters).length > 0 ? 'Try adjusting your search or filters.' : 'Get started by creating your first packing recipe.'}
                    />
                ) : (
                    <div className={loading ? 'opacity-60 transition-opacity p-2' : 'transition-opacity p-2'}>
                        <Table
                            columns={columns}
                            data={recipes}
                            onRowClick={(row) => navigate(`/production/packing-recipes/${row.id}`)}
                        />
                    </div>
                )}
            </Card>
            ...
            >
                <form onSubmit={handleCreate} className="space-y-4">
                    ...
                    <Input
                        ...
                        placeholder="e.g. Packing batch — 40mm cut pieces"
                        required
                    />
                    <div>
...
export default PackingRecipesPage;
