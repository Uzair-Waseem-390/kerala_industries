...
import { useCuttingRecipes } from '../../hooks/useProduction';
...
// Recipe list — Cutting is the second WIP stage (Rewound Cores -> cut-length
// pieces). Mirrors RecipesPage exactly; the create form only collects
// name + description since recipe_type is implied by the /cutting-recipes/
// endpoint itself.
const CuttingRecipesPage = () => {
    ...
    const handleCreate = async (e) => {
        ...
        try {
            ...
            resetForm();
            if (created?.id) {
                navigate(`/production/cutting-recipes/${created.id}`);
            }
        } catch (err) {
        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-2xl sm:text-3xl font-bold text-neutral-900">Cutting Recipes</h1>
                        <p className="text-neutral-500 mt-0.5 text-sm sm:text-base">
                            Cutting production batches — issue Rewound Cores, record cut lengths, finish to lock in cost
                        </p>
                    </div>
                </div>
                <Button
            ...
            <Card className="p-0 overflow-hidden" hover={false}>
                {recipes.length === 0 ? (
                    <EmptyState
                        ...
                        title="No cutting recipes found"
                        description={searchTerm || Object.keys(filters).length > 0 ? 'Try adjusting your search or filters.' : 'Get started by creating your first cutting recipe.'}
                    />
                ) : (
                    <div className={loading ? 'opacity-60 transition-opacity p-2' : 'transition-opacity p-2'}>
                        <Table
                            columns={columns}
                            data={recipes}
                            onRowClick={(row) => navigate(`/production/cutting-recipes/${row.id}`)}
                        />
                    </div>
                )}
            </Card>
            ...
            >
                <form onSubmit={handleCreate} className="space-y-4">
                    ...
                    <Input
                        ...
                        placeholder="e.g. Cutting batch — 40gsm cores"
                        required
                    />
                    <div>
...
export default CuttingRecipesPage;
