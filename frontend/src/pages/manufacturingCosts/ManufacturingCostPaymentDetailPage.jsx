...
import { Trash2, Calendar, User, Clock, StickyNote, Receipt, TrendingDown, Hash } from 'lucide-react';
...
const ENTITY_TYPE_LABELS = {
    ...
    machine: 'Factory Overhead — Machine',
    rent: 'Factory Overhead — Rent',
    electricity: 'Factory Overhead — Electricity',
};
...
const ManufacturingCostPaymentDetailPage = () => {
    const { id } = useParams();
    ...
    const backHref = payment?.entity
        ? `/manufacturing-costs/payable-entities/${payment.entity}`
    ...
    const fetchPayment = useCallback(async () => {
        ...
        setNotFound(false);
        try {
            const data = await manufacturingCostsApi.payments.getById(id);
            setPayment(data);
        } catch (error) {
    ...
    if (notFound) {
        return (
            <div className="text-center py-16">
                ...
                <BackLink to="/manufacturing-costs/payments" className="mt-4 inline-flex">Back to Payments</BackLink>
            </div>
        );
    }
    ...
    if (loadError || !payment) {
        return (
            <div className="space-y-4">
                <BackLink to="/manufacturing-costs/payments">Back to Payments</BackLink>
                <InlineAlert
                    variant="error"
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <BackLink to={backHref}>Back to {payment.entity_name || 'Record'}</BackLink>
                    <div className="flex items-center gap-3 mt-2">
                        <h1 className="text-2xl sm:text-3xl font-bold text-neutral-900">Payment Details</h1>
                        <Badge variant="neutral">{ENTITY_TYPE_LABELS[payment.entity_type] || payment.entity_type}</Badge>
                    </div>
                    <p className="text-neutral-500 text-sm sm:text-base mt-1">
                        {payment.reference_number} — {payment.entity_name}
                    </p>
                    <p className="mt-1 text-lg font-semibold text-error-600">
            ...
            <Card className="p-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-5">
                    <InfoField icon={Hash} label="Reference Number" value={payment.reference_number} />
                    <InfoField label="Amount" value={<span className="text-error-600">Rs. {fmt(payment.amount)}</span>} />
            ...
            <div className="flex gap-3 pt-4 border-t border-neutral-200">
                <Link to={backHref}>
                    <Button variant="secondary">Back to {payment.entity_name || 'Record'}</Button>
                </Link>
                <Button variant="danger" icon={Trash2} onClick={() => setDeleteConfirm(true)}>
                    Delete Payment
                </Button>
...
export default ManufacturingCostPaymentDetailPage;
