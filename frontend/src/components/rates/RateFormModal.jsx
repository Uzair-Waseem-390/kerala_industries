...
const RateFormModal = ({
    ...
    useEffect(() => {
        if (!isOpen || !product?.id || !productType) return undefined;
        let cancelled = false;
        setCost(null);
        setCostLoading(true);
        ratesApi.getCost(productType, product.id)
            .then((data) => { if (!cancelled) setCost(data); })
            .catch(() => { if (!cancelled) setCost(null); })
            .finally(() => { if (!cancelled) setCostLoading(false); });
        return () => { cancelled = true; };
    }, [isOpen, product?.id, productType]);
...
RateFormModal.propTypes = {
    ...
    productType: PropTypes.oneOf(['rm', 'fg']),
